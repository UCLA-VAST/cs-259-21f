...
ostream& operator<<(ostream& os, const Update& obj) {
  return os << "{dst: " << obj.dst << ", depth: " << obj.depth << "}";
}
...
update_offset_init:
  ...
  update_config_q.try_open();  // Restore the state of stream.
  ...
  TAPA_WHILE_NOT_EOT(update_req_q) {
    ...
    if (update_req.phase == TaskReq::kScatter) {
      TAPA_WHILE_NOT_EOT(update_in_q) {
        ...
        updates[update_offset] = tapa::bit_cast<bits<Update>>(update);
...
#pragma HLS pipeline II = 1
        Eid update_offset = update_offsets[pid] + update_idx;
        const auto update = tapa::bit_cast<Update>(updates[update_offset]);
  ...
  update_req_q.try_open();  // Restore the state of stream.
...
#pragma HLS pipeline II = 1
        auto edge = tapa::bit_cast<Edge>(edges[req.eid_offset + eid]);
        auto vertex_attr = vertices_local[edge.src - req.base_vid];
        Update update;
        update.dst = edge.dst;
        update.depth = vertex_attr;
    ...
    gather:
      TAPA_WHILE_NOT_EOT(update_in_q) {
        ...
        if (update.depth < old_vertex_value) {
          vertices_local[idx] = update.depth;
          active = true;
        }
      }
      update_in_q.open();
    store:
      for (Vid i = 0; i < req.num_vertices; ++i) {
#pragma HLS pipeline II = 1
        vertices[req.vid_offset + i] = vertices_local[i];
      }
    }
    TaskResp resp{req.phase, req.pid, active};
    resp_q.write(resp);
  }
  req_q.try_open();  // Restore the state of stream.
